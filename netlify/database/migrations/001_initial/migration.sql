...
--
-- All timestamps are ISO-8601 UTC strings, so this file is valid on both SQLite (local
-- dev) and Postgres (Netlify DB) without a dialect switch. Keep it that way — SQLite
-- PRAGMAs live in the driver, not here.
--
-- netlify/database/migrations/001_initial/migration.sql is a copy of this file, and
-- `npm run check:schema` fails the build if the two drift apart.
